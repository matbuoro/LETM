...
BESTmcmc = function( y1, y2, numSavedSteps=100000, thinSteps=1, showMCMC=TRUE) { 
...
BESTsummary = function( M,N, mcmcChain ) {
  source("HDIofMCMC.R")
  ...
  for (i in 1:M){
    summaryInfo[ mu[i] , ] = mcmcSummary( mcmcChain[,mu[i]] ) 
    summaryInfo[ sigma[i] , ] = mcmcSummary( mcmcChain[,sigma[i]] ) 
  }
